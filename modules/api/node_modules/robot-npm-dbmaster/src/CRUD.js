...
class CRUD {
    ...
    _sender
    constructor(url) {
        this._Url = `${url}/api/use`;
        this._sender = new Sender(this._Url);
    }

    create = async (Model, inData) => {
        const { status, data } = await this._sender.POST([ Model ], { data: inData })
        return { status, data };
    }

    getOne = async (Model, by, value) => {
        return await this._sender.GET('one', [ Model ], [{ key: 'by', value: by }, { key: 'value', value }]);
    }
